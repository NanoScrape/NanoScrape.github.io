...
| `handoff/catalog-header.html` | paste into `index.html` | Catalog header + working filter chips (replaces the `Actor Catalog` h2 + subtitle). No per-card edits needed. |
| `handoff/PATCH-NOTES.md` | read first if already deployed | Cache-busting the script URLs, and the catalog header that was missing from the first handoff. |
...
The `?v=` query is required — without it browsers and the CDN keep serving the previously
cached `components.js` and the old header/footer stay on screen. Bump the number on every
